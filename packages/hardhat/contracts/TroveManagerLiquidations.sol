...
contract TroveManagerLiquidations is TroveManagerBase, ITroveManagerLiquidations {
    ...
    struct LiquidationTotals {
        ...
        uint256 totalCUSDGasCompensation;
    ...
    struct LocalVariables_LiquidationSequence {
        uint256 remainingCUSDInStabPool;
        uint256 i;
        uint256 ICR;
        address user;
    ...
    function setAddresses(
        ...
        bankOfCronosTreasury = _bankOfCronosTreasury;
    ...
    function batchLiquidateTroves(address[] memory _troveArray, address _liquidator) external override {
        ...
        // Move liquidated Collateral and CUSD to the appropriate pools
        ...
        emit Liquidation(
            vars.liquidatedDebt,
            totals.totalCUSDGasCompensation,
    ...
    function _getTotalFromBatchLiquidate_RecoveryMode(
        ...
        for (vars.i = 0; vars.i < troveArrayLen; ++vars.i) {
            ...
            if (!vars.backToNormalMode) {
                ...
                if (vars.ICR >= MCR && vars.remainingCUSDInStabPool == 0) {
                    continue;
                }
                ...
                singleLiquidation = _liquidateRecoveryMode(
                    ...
                    vars.user,
                    vars.ICR,
                    vars.remainingCUSDInStabPool,
                    TCR
                );

                // Update aggregate trackers
                vars.remainingCUSDInStabPool = vars.remainingCUSDInStabPool.sub(
    ...
    function _liquidateNormalMode(
        ...
        singleLiquidation.CUSDGasCompensation = CUSD_GAS_COMPENSATION;
        ...
        // accrue to Bank of Cronos Treasury until the assets are claimed
        _updateWAssetsRewardOwner(collsToUpdate, _borrower, bankOfCronosTreasury);
    ...
    function _liquidateRecoveryMode(
        ...
        if (_ICR <= _100pct) {
            ...
            // Bank of Cronos Treasury until the WAssets are claimed by troves
            _updateWAssetsRewardOwner(singleLiquidation.collToRedistribute, _borrower, bankOfCronosTreasury);
             ...
             * and there is CUSD in the Stability Pool, only offset, with no redistribution,
             * but at a capped rate of 1.1 and only if the whole debt can be liquidated.
             * The remainder due to the capped rate will be claimable as collateral surplus.
             * ICR >= 110% is implied from last else if statement. 
             */
        } else if (
           (_ICR < _TCR) && (singleLiquidation.entireTroveDebt <= _CUSDInStabPool)
        ) {
            troveManager.movePendingTroveRewardsToActivePool(
                _activePool,
                _defaultPool,
            ...
            require(_CUSDInStabPool != 0, "TML: zero CUSD in Stab Pool");
            ...
            // if (_ICR >= MCR && ( _ICR >= _TCR || singleLiquidation.entireTroveDebt > _CUSDInStabPool))
     ...
     * will be length equal to _colls.tokens and _colls.amounts. However, if no colls are redistributed to stability pool (which is the case when _CUSDInStabPool == 0),
    ...
    function _getOffsetAndRedistributionVals(
        ...
        if (_CUSDInStabPool != 0) {
             ...
             *
             *  If the trove's debt is larger than the deposited CUSD in the Stability Pool:
             *
             *  - Offset an amount of the trove's debt equal to the CUSD in the Stability Pool
            ...
            or_vals.debtToOffset = LiquityMath._min(_entireTroveDebt, _CUSDInStabPool);
            ...
            // $110 of collateral for every 100 CUSD they are using to liquidate.
            ...
            // giving stability pool holders $110 of collateral for every 100 CUSD that is taken from them,
            ...
            // all colls are redistributed because no CUSD in stability pool to liquidate
    ...
    function _addLiquidationValuesToTotals(
        ...
        newTotals.totalCUSDGasCompensation = oldTotals.totalCUSDGasCompensation.add(
            singleLiquidation.CUSDGasCompensation
        );
    ...
    */
    ...
    function _sendGasCompensation(
        ...
        if (_CUSD != 0) {
            CUSDTokenContract.returnFromPool(gasPoolAddress, _liquidator, _CUSD);
        }
    ...
    /*
     * Update rewards tracking so future rewards go to Bank of Cronos Treasury
